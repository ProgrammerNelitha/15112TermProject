...
def game_onScreenActivate(app):
    ...
    #Says what legal values are banned in each region, as pointed out by hint 2. Include exception cells
    app.bannedLegalValues=dict()
...
def game_onKeyPress(app,key):
    ...
    if key=='l':
        if app.hint2Coords!=[]:
            if str(app.hint2Region) not in app.bannedLegalValues:
                app.bannedLegalValues[str(app.hint2Region)]=(set(),app.hint2Combo)
            for val in app.hint2Legals:
                app.bannedLegalValues[str(app.hint2Region)][0].add(val)
            
            for (a,b) in app.hint2Coords:                          
                s=app.legals[(a,b)]
                app.legals[(a,b)] = s-app.hint2Legals
                print('Hint 2 Edits:',s,app.hint2Legals,app.legals[(a,b)])
    ...
    for row in range(app.rows):
        for col in range(app.cols):
            if app.board[row][col]!=0:
                ...
                #First find the legal values it cant be
                badLegals=set()
                for region in app.bannedLegalValues:
                    if f'{row,col}' in region:
                        if (row,col) not in app.bannedLegalValues[region][1]:
                            badLegals=badLegals|app.bannedLegalValues[region][0]

                for legalVal in app.legals[(row,col)]:
                    if legalVal not in badLegals:
