...
def help_redrawAll(app):
    ...
    drawLabel("There are two hints a player can ask for. The first hint directs a player to a cell with only 1 legal value in it.",app.width/2,360,size=20)
    drawLabel("The second hint directs the player to eliminate certain legal values from cells,",app.width/2,390,size=20) 
    drawLabel("based on the legal values in cells within the same region.",app.width/2,420,size=20)

    drawLabel("Use the mouse to select a cell, the keyboard to enter numbers, and 'backspace' to delete said numbers",app.width/2,490,size=20)
    ...
    drawLabel("Additionally, with Manual Entry Mode you can use the keyboard to directly enter legal values for any cell.",app.width/2,580,size=20)
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
...
def game_onMousePress(app,mouseX,mouseY):
    ...
    if 695<=mouseX<=945 and 263<=mouseY<=303:
...
def game_onKeyPress(app,key):
    if app.selectedCellX!=None:
        if not app.manualLegals:
            ...
            #In manual legal mode, user can remove legals values, but can't add illegal legal values
            if key.isdigit():
                default=getDefaultLegals(app.board,app.selectedCellX,app.selectedCellY)
                val = int(key)
                if val in app.legals[(app.selectedCellX,app.selectedCellY)]:    
                    banned=app.bannedLegals.get((app.selectedCellX,app.selectedCellY),set())
                    banned.add(val)
                    app.bannedLegals[(app.selectedCellX,app.selectedCellY)]=banned
                else:
                    if val in default:
                        app.bannedLegals[(app.selectedCellX,app.selectedCellY)].remove(val)
        resetLegals(app)
...
def game_redrawAll(app):
    ...
    drawRect(820,283,250,40,fill=None,border='black',align='center')
    if app.manualLegals:
        drawLabel("Manual Legals Entry",820,283,size=25,bold=True, fill='green')
    else:
        drawLabel("Manual Legals Entry",820,283,size=25,bold=True, fill='red')
